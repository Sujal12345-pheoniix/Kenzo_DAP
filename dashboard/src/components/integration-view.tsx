...
export class AppComponent implements OnInit {
  ...
  // Custom high-performance tokenized syntax display (HTML/JS code colors without libraries)
  const renderHighlightedCode = (code: string) => {
    // Basic formatting mapping keywords, comments, strings
    const lines = code.split('\n');
    return lines.map((line, idx) => {
      let isComment = line.trim().startsWith('//') || line.trim().startsWith('<!--') || line.trim().endsWith('-->');
      
      if (isComment) {
        return <div key={idx} className="text-zinc-500 font-mono">{line}</div>;
      }
        ...
        <div key={idx} className="text-zinc-300 font-mono">
          {line.split(/([{}()<>=".,:;'"\s]|\bimport\b|\bexport\b|\bconst\b|\bif\b|\blet\b|\bfunction\b|\btypeof\b|\bdocument\b|\bwindow\b)/).map((part, pidx) => {
            ...
            if (part === 'Kenzo' || part === 'document' || part === 'window') {
              return <span key={pidx} className="text-violet-400 font-semibold">{part}</span>;
            }
            ...
            if (part === 'apiKey' || part === 'apiBaseUrl') {
              return <span key={pidx} className="text-rose-400 font-medium">{part}</span>;
            }
